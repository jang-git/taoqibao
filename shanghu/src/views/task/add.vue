            ...
            <div>
              <Card>
                ...
                <div v-if="taskKeywords.keywords1.ischeck" class="searchnew">
                  <!-- <Form ref="form2"  :model="keywords1" :label-width="110">
                    <FormItem v-for="(item,index) in keywords1.items" :key="index" :label="'搜索关键词' + (index+1)" :prop="'items.' + index + '.keyword'" :rules="{required: true, message: '搜索关键词 ' + (index+1) +' 不能为空', trigger: 'blur'}">
                      <Input type="text" v-model="item.keyword" style="width:180px;"></Input>
                      <InputNumber :max="10000" :min="1" v-model="item.num"></InputNumber>单
              ...
              <Card class="margin-top-10">
                ...
                <div v-if="taskKeywords.keywords2.ischeck" class="searchnew">
                  <!-- <Form ref="form3"  :model="keywords2" :label-width="110">
                    <FormItem v-for="(item,index) in keywords2.items" :key="index" :label="'搜索关键词' + (index+1)" :prop="'items.' + index + '.keyword'" :rules="{required: true, message: '搜索关键词 ' + (index+1) +' 不能为空', trigger: 'blur'}">
                      <Input type="text" v-model="item.keyword" style="width:180px;"></Input>
                      <InputNumber :max="1" :min="1" v-model="item.num"></InputNumber>单
                      <Button v-if="index>0" @click="delkeywords1(index)" type="error" size="small">删除</Button>
